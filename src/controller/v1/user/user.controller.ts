...
class UserController {
    ...
    async createNewUser(req: any, res: any) {
        const required = reqValidator(req, ["fullName", "password", "email", "role"]);
        if (!required.status) {
        ...
        //TODO: check if rootDirectory is valid

        // check if fileControlLevel is valid

        // check if email exist on user and members
        const existingUser = await userService.findUserbyEmail(req.body.email)
        if (existingUser) throw new AppError({message: "user with this email exist", statusCode: 409})
        ...
        if (!newMember) {
            throw new AppError({message: "Failed to create new member", statusCode: 500});
        }
